...
if gendata == True:
    ...
    #Generation of wave velocity parameters


    #Number of different wave velocity parameters
    trainsamp = 10 
    alpharange = np.random.normal(0,1,trainsamp) #1,1
    betarange = np.random.normal(0,1,trainsamp) #1,1
    gammarange = np.random.normal(0,1,trainsamp) #80,20

    coord = np.load("gauss_ellipse_coord.npy") #NOTE need to have coordinates a pirori for simulation to work
    coord = np.load("gauss_polygon_coord.npy")
    ...
    for i in range(trainsamp): #Simulate wave equation for different wave velocity parameters
        ...
        u_n_n = interpolate(Constant(0),V)
        
        #set wave velocity parameter
        c = interpolate(Constant(0),V)
        c.vector()[:] = incondp[i].squeeze()[:]
        #c = 5
        ...
        for i in range(num_steps):
            ...
            img = axs[i].scatter(coord[:,0],coord[:,1],c = field_val[i], vmin = np.min(field_val[-1]), vmax = np.max(field_val[-1]))
            axs[i].set_xlabel(" X ")
            axs[i].set_ylabel(" Y ")
            plt.colorbar(img)
    ...
    for i in range(num_steps):
        ...
        axs[i].set_title("(Param: (" + str(i) + ")")
        img = axs[i].scatter(coord[:,0],coord[:,1],c = field_val[i])
        axs[i].set_xlabel(" X ")
        axs[i].set_ylabel(" Y ")
        plt.colorbar(img)
